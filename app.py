...
# GitHub에 올릴 예측 파일명
# 파일명이 다르면 여기만 변경하면 됩니다.
PRED_FILES = {
    "overall": BASE_DIR / "predictions_v5-overall.npz",  # 일반 모델
    "peak": BASE_DIR / "predictions_v5-peak.npz",        # 피크 모델
}
MODEL_LABELS = {
    "overall": "일반 모델",
    "peak": "피크 모델",
...
def kwh_to_color(value: float, vmin: float, vmax: float) -> list[int]:
    ...
    ratio = 0.5 if vmax <= vmin else float(np.clip((value - vmin) / (vmax - vmin), 0.0, 1.0))
    return [int(205 - 155 * ratio), int(222 - 135 * ratio), int(247 - 25 * ratio), 190]
...
def get_recent_chat_context(messages: list[dict], limit: int = 6) -> list[dict]:
    ...
    return [{"role": m.get("role", "assistant"), "content": str(m.get("content", ""))[:700]} for m in recent]
...
# 예측 파일 로딩
# =========================================================
def pick_npz_array(npz: np.lib.npyio.NpzFile, candidates: list[str]) -> Optional[np.ndarray]:
...
def normalize_prediction_array(arr: np.ndarray) -> np.ndarray:
    ...
    return np.clip(arr, 0, None).astype(float)


def build_long_prediction_df(pred_arr: np.ndarray, meta: Dict, true_arr: Optional[np.ndarray] = None) -> pd.DataFrame:
    zone_ids = list(meta["zone_ids"])
    n_zones_meta = len(zone_ids)

    pred_arr = normalize_prediction_array(pred_arr)
    n_samples, n_horizons, n_zones = pred_arr.shape
    if n_zones != n_zones_meta:
        raise ValueError(f"예측 결과의 zone 수({n_zones})와 meta.json의 zone_ids 수({n_zones_meta})가 다릅니다.")

    true_norm = None
    if true_arr is not None:
        true_norm = normalize_prediction_array(true_arr)
        if true_norm.shape != pred_arr.shape:
            raise ValueError(f"예측 shape {pred_arr.shape}와 실제값 shape {true_norm.shape}가 다릅니다.")
    ...
    if true_norm is not None:
        df["y_true_kwh"] = true_norm.reshape(-1)

    zone_map = pd.DataFrame({"zone_idx": list(range(n_zones_meta)), "생활권역ID": zone_ids})
    df = df.merge(zone_map, on="zone_idx", how="left")
    ...
    df["target_global_time_idx"] = df["global_time_idx"] + df["horizon"].astype(int)

    df["base_datetime"] = START_DATETIME + pd.to_timedelta(df["global_time_idx"] * TIME_UNIT_MINUTES, unit="m")
    df["target_datetime"] = START_DATETIME + pd.to_timedelta(df["target_global_time_idx"] * TIME_UNIT_MINUTES, unit="m")

    df["date_str"] = df["base_datetime"].dt.strftime("%Y-%m-%d")
    df["time_str"] = df["base_datetime"].dt.strftime("%H:%M")
    df["target_date_str"] = df["target_datetime"].dt.strftime("%Y-%m-%d")
    df["target_time_str"] = df["target_datetime"].dt.strftime("%H:%M")
    df["hour"] = df["base_datetime"].dt.hour
    df["minute"] = df["base_datetime"].dt.minute
...
def load_prediction_file(pred_path: Path, meta: Dict) -> pd.DataFrame:
    if not pred_path.exists():
        raise FileNotFoundError(f"예측 파일을 찾을 수 없습니다: {pred_path.name}")
    ...
    if suffix == ".npz":
        ...
        return build_long_prediction_df(pred_arr=pred_arr, true_arr=true_arr, meta=meta)

    if suffix == ".npy":
        pred_arr = np.load(pred_path)
        return build_long_prediction_df(pred_arr=pred_arr, true_arr=None, meta=meta)
    ...
    if suffix == ".csv":
        ...
        raise ValueError(f"지원하지 않는 예측 파일 형식입니다: {pred_path.name}")

    required = {"sample_idx", "zone_idx"}
    missing = required - set(raw.columns)
    if missing:
        raise ValueError(f"예측 파일에 필수 컬럼이 없습니다: {missing}")

    pred_candidates = ["predicted_kwh", "y_pred_kwh", "pred_kwh", "prediction_kwh", "pred", "y_pred"]
    true_candidates = ["y_true_kwh", "true_kwh", "actual_kwh", "true", "y_true"]
    pred_col = next((c for c in pred_candidates if c in raw.columns), None)
    true_col = next((c for c in true_candidates if c in raw.columns), None)
    if pred_col is None:
        raise ValueError("예측 CSV/Parquet에 predicted_kwh 또는 y_pred_kwh 컬럼이 필요합니다.")

    raw = raw.copy()
    raw["sample_idx"] = pd.to_numeric(raw["sample_idx"], errors="coerce").astype(int)
    raw["zone_idx"] = pd.to_numeric(raw["zone_idx"], errors="coerce").astype(int)
    raw[pred_col] = pd.to_numeric(raw[pred_col], errors="coerce").fillna(0).clip(lower=0)
    if "horizon" not in raw.columns:
        raw["horizon"] = 0
    raw["horizon"] = pd.to_numeric(raw["horizon"], errors="coerce").fillna(0).astype(int)

    zone_ids = list(meta["zone_ids"])
    zone_map = pd.DataFrame({"zone_idx": list(range(len(zone_ids))), "생활권역ID": zone_ids})
    raw = raw.merge(zone_map, on="zone_idx", how="left")
    raw = raw.rename(columns={pred_col: "predicted_kwh"})
    if true_col is not None:
        raw["y_true_kwh"] = pd.to_numeric(raw[true_col], errors="coerce").fillna(0).clip(lower=0)

    val_end = int(meta.get("val_end", 0))
    look_back = int(meta.get("look_back", 0))
    test_time_offset = val_end + look_back
    raw["global_time_idx"] = raw["sample_idx"] + test_time_offset
    raw["target_global_time_idx"] = raw["global_time_idx"] + raw["horizon"]
    raw["base_datetime"] = START_DATETIME + pd.to_timedelta(raw["global_time_idx"] * TIME_UNIT_MINUTES, unit="m")
    raw["target_datetime"] = START_DATETIME + pd.to_timedelta(raw["target_global_time_idx"] * TIME_UNIT_MINUTES, unit="m")
    raw["date_str"] = raw["base_datetime"].dt.strftime("%Y-%m-%d")
    raw["time_str"] = raw["base_datetime"].dt.strftime("%H:%M")
    raw["target_date_str"] = raw["target_datetime"].dt.strftime("%Y-%m-%d")
    raw["target_time_str"] = raw["target_datetime"].dt.strftime("%H:%M")
    raw["hour"] = raw["base_datetime"].dt.hour
    raw["minute"] = raw["base_datetime"].dt.minute
    raw["daily_slot"] = raw["hour"] * 2 + raw["minute"] // 30
    return raw


@st.cache_data(show_spinner="전체 예측 결과 로딩 중...")
def load_all_predictions(meta: Dict) -> Dict[str, pd.DataFrame]:
    return {mode: load_prediction_file(path, meta) for mode, path in PRED_FILES.items()}
...
def llm_extract_query(text: str, pred: pd.DataFrame, area_info: pd.DataFrame) -> dict:
...
"""
    ...
    if re.search(r"어떻게|작동|원리|방식|설명|사용법|뭐야|무엇|데이터|모델|지도", text):
        intent, needs = "service_explanation", False
    elif re.search(r"안녕|하이|hello|hi", text, re.IGNORECASE):
        intent, needs = "greeting", False
    elif re.search(r"수요|충전|예측|전기차|생활권|지도|보여|알려|조회|분석|혼잡|급증|피크|kwh|kw", text, re.IGNORECASE):
        intent, needs = "demand_lookup", True
    else:
        intent, needs = "other", False
    return {"date_text": None, "time_text": None, "location_text": None, "intent": intent, "needs_data_lookup": needs, "raw": llm_text}
    ...
    for _, row in area_info.iterrows():
        ...
        if m:
            ...
            if gu in clean_text(label) or gu in clean_text(dongs) or gu in clean_text(raw_label):
                score += 8
        if score > best_score:
            best_score, best_zone = score, zone_id
    return best_zone if best_score > 0 else None


def parse_user_query(text: str, pred: pd.DataFrame, area_info: pd.DataFrame, fallback_date: str, fallback_time: str, fallback_zone_id: str) -> Dict:
    available_dates = sorted(pred["date_str"].unique())
    ...
    if any_date and any_date not in available_dates:
        return {"ok": False, "reason": "date_unavailable", "message": f"{any_date} 날짜는 현재 예측 데이터셋에 존재하지 않습니다. 현재 조회 가능한 날짜 범위는 {available_dates[0]}부터 {available_dates[-1]}까지입니다. 예: {FIXED_QUERY_EXAMPLE}", "date": fallback_date, "time": fallback_time, "zone_id": fallback_zone_id, "llm_extract": llm_result}

    parsed_date = parse_date_from_text(date_source, available_dates) or parse_date_from_text(text, available_dates)
    if parsed_date is None:
        return {"ok": False, "reason": "missing_date", "message": f"조회할 날짜를 찾지 못했습니다. 연도, 월, 일을 모두 포함해 입력해 주세요. 현재 데이터는 {available_dates[0]}부터 {available_dates[-1]}까지 조회할 수 있습니다. 예: {FIXED_QUERY_EXAMPLE}", "date": fallback_date, "time": fallback_time, "zone_id": fallback_zone_id, "llm_extract": llm_result}
...
# 데이터 조회
# =========================================================
def get_base_slice(pred: pd.DataFrame, date: str, time: str, horizon: int) -> pd.DataFrame:
    slice_df = pred[(pred["date_str"] == date) & (pred["time_str"] == time) & (pred["horizon"] == horizon)].copy()
    if slice_df.empty and horizon != 0:
        slice_df = pred[(pred["date_str"] == date) & (pred["time_str"] == time) & (pred["horizon"] == 0)].copy()
    return slice_df


def get_zone_horizon(pred: pd.DataFrame, date: str, time: str, zone_id: str) -> pd.DataFrame:
    return pred[(pred["date_str"] == date) & (pred["time_str"] == time) & (pred["생활권역ID"] == zone_id)].copy()


def calc_zone_metrics(pred: pd.DataFrame, date: str, time: str, zone_id: str) -> Dict[str, Any]:
    zone_horizon = get_zone_horizon(pred, date, time, zone_id)
    if zone_horizon.empty:
        return {"current_kwh": 0.0, "pred_30m_kwh": 0.0, "rank": 0, "n_zones": pred["zone_idx"].nunique(), "peak_time": "-", "peak_kwh": 0.0, "low_time": "-", "low_kwh": 0.0}

    h0 = zone_horizon[zone_horizon["horizon"] == 0]
    if h0.empty:
        h0 = zone_horizon.sort_values("horizon").head(1)
    h1 = zone_horizon[zone_horizon["horizon"] == PREDICT_HORIZON_30M]
    if h1.empty:
        h1 = zone_horizon.sort_values("horizon").head(1)

    current_row = h0.iloc[0]
    pred_30m_row = h1.iloc[0]
    if "y_true_kwh" in zone_horizon.columns and pd.notna(current_row.get("y_true_kwh", np.nan)):
        current_kwh = float(current_row["y_true_kwh"])
    else:
        current_kwh = float(current_row["predicted_kwh"])
    pred_30m_kwh = float(pred_30m_row["predicted_kwh"])

    rank_slice = get_base_slice(pred, date, time, PREDICT_HORIZON_30M)
    zone_rank = 0
    if not rank_slice.empty:
        z = rank_slice[rank_slice["생활권역ID"] == zone_id]
        if not z.empty:
            zone_rank = int(rank_slice["predicted_kwh"].rank(method="min", ascending=False).loc[z.index[0]])

    window = zone_horizon.sort_values("horizon").head(PEAK_WINDOW_HORIZONS).copy()
    peak_row = window.loc[window["predicted_kwh"].idxmax()]
    low_row = window.loc[window["predicted_kwh"].idxmin()]
    return {
        "current_kwh": current_kwh,
        "pred_30m_kwh": pred_30m_kwh,
        "rank": zone_rank,
        "n_zones": pred["zone_idx"].nunique(),
        "peak_time": str(peak_row["target_time_str"]),
        "peak_kwh": float(peak_row["predicted_kwh"]),
        "low_time": str(low_row["target_time_str"]),
...
def prepare_map_payload(map_gdf: gpd.GeoDataFrame, use_3d_column: bool, focus_zone_id: Optional[str], previous_focus_zone_id: Optional[str]) -> dict:
    ...
    def calc_elevation(row: pd.Series) -> float:
        ...
        scaled_height = 2450.0 * ratio + (450.0 if bool(row.get("is_focus", False)) else 0.0)
        return float(np.clip(220.0 + scaled_height, 220.0, 3100.0))
    ...
    return {"geojson": geojson, "columns": columns, "overview_view": overview_view, "previous_view": previous_view, "target_view": target_view, "use_3d_column": bool(use_3d_column), "has_focus": has_focus, "has_previous_focus": has_previous_focus}
...
def render_deck_map_html(payload: dict, animate: bool, height: int) -> None:
    ...
    <body>
        ...
        <script>
            ...
            function makeView(start, target, t) {{ const e = smootherstep(t); return {{ latitude: lerp(start.latitude, target.latitude, e), longitude: lerp(start.longitude, target.longitude, e), zoom: lerp(start.zoom, target.zoom, e), pitch: lerp(start.pitch || 0, target.pitch || 0, e), bearing: lerp(start.bearing || 0, target.bearing || 0, e) }}; }}
            function polygonLayer() {{ return new deck.GeoJsonLayer({{ id: "living-area-polygons", data: geojsonData, pickable: true, stroked: true, filled: true, extruded: false, getFillColor: f => f.properties.fill_color || [220, 225, 232, 90], getLineColor: f => f.properties.line_color || [255, 255, 255, 180], getLineWidth: f => f.properties.line_width || 20, lineWidthMinPixels: 1.2, autoHighlight: true }}); }}
            function columnLayer() {{ return new deck.ColumnLayer({{ id: "living-area-columns", data: columnData, diskResolution: 32, radius: 250, extruded: true, pickable: true, getPosition: d => [d.lon, d.lat], getFillColor: d => d.fill_color || [100, 140, 230, 190], getElevation: d => d.elevation || 0, elevationScale: 1, autoHighlight: true }}); }}
            ...
            const deckgl = new deck.DeckGL({{ container: "map", mapStyle: "https://basemaps.cartocdn.com/gl/positron-gl-style/style.json", viewState: currentView, controller: true, layers: makeLayers(), onViewStateChange: e => {{ currentView = e.viewState; deckgl.setProps({{ viewState: currentView }}); }}, getTooltip: info => {{ if (!info.object) return null; const p = info.object.properties || info.object; const label = p["생활권역표시명"] || p.ID || ""; const id = p.ID || ""; const kwh = p.predicted_kwh || ""; const targetTime = p.target_time_str || ""; const dongs = p["행정동명목록"] || ""; return {{ html: `<div style="font-family: Inter, sans-serif;"><b>${{label}}</b><br/><span style="color:#B7C1D3;">생활권ID:</span> ${{id}}<br/><span style="color:#B7C1D3;">예측 시각:</span> ${{targetTime}}<br/><span style="color:#B7C1D3;">예측 충전량:</span> <b>${{kwh}} kWh</b><br/><span style="color:#B7C1D3;">행정동:</span> ${{dongs}}</div>` }}; }} }});
            function animateBetween(startView, endView, duration) {{ return new Promise(resolve => {{ const startTime = performance.now(); function step(now) {{ const raw = (now - startTime) / duration; const t = Math.min(Math.max(raw, 0), 1); const nextView = makeView(startView, endView, t); deckgl.setProps({{ viewState: nextView, layers: makeLayers() }}); if (t < 1) {{ requestAnimationFrame(step); }} else {{ deckgl.setProps({{ viewState: endView, layers: makeLayers() }}); resolve(); }} }} requestAnimationFrame(step); }}); }}
            async function runAnimation() {{ if (!(shouldAnimate && hasFocus)) return; await new Promise(resolve => setTimeout(resolve, 220)); if (hasPreviousFocus) {{ await animateBetween(previousView, overviewView, 1050); await new Promise(resolve => setTimeout(resolve, 120)); await animateBetween(overviewView, targetView, 1250); }} else {{ await animateBetween(overviewView, targetView, 2200); }} }}
            runAnimation();
        </script>
    </body>
    </html>
...
def draw_alerts_stack(top_df: pd.DataFrame, selected_time: str) -> None:
    ...
    <html><head><meta charset="utf-8" /><style>
        ...
        .alert-brand {{ font-family: "Holtwood One SC", Georgia, serif; font-size: 11px; font-weight: 400; letter-spacing: -0.02em; line-height: 1; margin-bottom: 8px; }}
        ...
        .right-label.time {{ margin-top: 8px; }}
        .right-value {{ color: #111111; font-size: 13px; font-weight: 750; line-height: 1.15; margin-top: 5px; white-space: nowrap; letter-spacing: -0.02em; }}
    </style></head><body>
...
def build_selected_detail_html(selected_label: str, selected_zone_id: str, selected_dongs: str, metrics: Dict[str, Any], selected_date: str, selected_time: str, model_label: str) -> str:
    return f"""
    <div class="selected-detail-card">
        ...
        <div class="detail-card-meta"><div class="meta-block"><div class="meta-label">조회 시각</div><div class="meta-text">{escape_html(selected_date)} {escape_html(selected_time)}</div></div><div class="meta-block"><div class="meta-label">포함 행정동</div><div class="meta-text">{escape_html(selected_dongs if selected_dongs else '행정동 정보 없음')}</div></div></div>
        <div class="detail-metric-grid">
            <div class="detail-metric"><div class="metric-label">현재 수요</div><div class="metric-value">{metrics['current_kwh']:.1f} kWh</div></div>
            <div class="detail-metric"><div class="metric-label">예측 수요</div><div class="metric-value">{metrics['pred_30m_kwh']:.1f} kWh</div><div class="metric-delta">30분 후</div></div>
            <div class="detail-metric"><div class="metric-label">피크 시간</div><div class="metric-value">{escape_html(metrics['peak_time'])}</div><div class="metric-delta">↑ {metrics['peak_kwh']:.1f} kWh</div></div>
            <div class="detail-metric"><div class="metric-label">여유 시간</div><div class="metric-value">{escape_html(metrics['low_time'])}</div><div class="metric-delta low">↓ {metrics['low_kwh']:.1f} kWh</div></div>
        </div>
    </div>
    """
...
def render_chat_panel(messages: list[dict], selected_detail_html: str | None = None, is_typing: bool = False) -> None:
    ...
    <!DOCTYPE html><html><head><meta charset="utf-8" /><style>
        ...
        .typing-bubble {{ display: inline-flex; align-items: center; gap: 5px; min-width: 54px; height: 34px; padding: 9px 13px; }} .typing-dot {{ width: 7px; height: 7px; border-radius: 999px; background: #1F78B4; display: inline-block; animation: typingBounce 1.05s infinite ease-in-out; }} .typing-dot.dot-1 {{ animation-delay: 0s; }} .typing-dot.dot-2 {{ animation-delay: 0.16s; }} .typing-dot.dot-3 {{ animation-delay: 0.32s; }} @keyframes typingBounce {{ 0%, 80%, 100% {{ transform: translateY(0); opacity: 0.42; }} 40% {{ transform: translateY(-7px); opacity: 1; }} }}
        ...
        .detail-card-kicker {{ color: #1F78B4; font-family: "Holtwood One SC", Georgia, serif; font-size: 10.5px; font-weight: 400; letter-spacing: -0.02em; margin-bottom: 5px; }} .detail-card-title {{ color: #111111; font-size: 18px; font-weight: 850; letter-spacing: -0.045em; line-height: 1.25; }} .detail-card-id {{ color: #5F666F; font-size: 11px; font-weight: 600; margin-top: 4px; }}
...
def build_fallback_answer(selected_date: str, selected_time: str, selected_label: str, metrics: Dict[str, Any], model_label: str) -> str:
    return (
        ...
        f"지도와 알림도 현재 선택된 {model_label} 기준으로 표시됩니다."
    )


def build_conversational_answer(user_text: str, reason_message: str, reason: str, llm_extract: dict | None, messages: list[dict], pred: pd.DataFrame, model_label: str) -> str:
    available_dates = sorted(pred["date_str"].unique())
...
- 모도리 패널 상단에서 일반 모델과 피크 모델을 전환할 수 있다.
...
"""
    ...
    if reason == "greeting" or re.search(r"안녕|하이|hello|hi", user_text, re.IGNORECASE):
        return f"안녕하세요. 저는 모도리입니다. 현재는 {model_label} 기준으로 답변합니다.\n\n예: {FIXED_QUERY_EXAMPLE}"
...
"""
    user_prompt = f"""
최근 대화:
{json.dumps(get_recent_chat_context(messages), ensure_ascii=False, indent=2)}
...
def build_answer_from_parsed_query(clean_user_text: str, parsed: Dict, pred: pd.DataFrame, area_info: pd.DataFrame, messages: list[dict], model_label: str) -> tuple[str, dict]:
    next_date, next_time, next_zone_id = parsed["date"], parsed["time"], parsed["zone_id"]
    metrics = calc_zone_metrics(pred, next_date, next_time, next_zone_id)
    next_selected_area = area_info[area_info["생활권역ID"] == next_zone_id].copy()
    if next_selected_area.empty:
        next_selected_label, next_selected_dongs = next_zone_id, ""
    else:
        next_selected_label = next_selected_area["생활권역표시명"].iloc[0]
        next_selected_dongs = next_selected_area["행정동명목록"].iloc[0] if "행정동명목록" in next_selected_area.columns else ""
    next_pred_filtered = get_base_slice(pred, next_date, next_time, PREDICT_HORIZON_30M)
    next_top10 = next_pred_filtered.sort_values("predicted_kwh", ascending=False).head(10)
    next_top10 = next_top10.merge(area_info[["생활권역ID", "생활권역라벨", "생활권역표시명"]], on="생활권역ID", how="left")
    answer = build_llm_answer(clean_user_text, next_date, next_time, next_selected_label, next_zone_id, next_selected_dongs, metrics, next_top10, messages, model_label)
...
try:
    ...
    predictions_by_mode = load_all_predictions(meta)
...
pred = predictions_by_mode[model_mode]
...
available_times_default = sorted(pred[pred["date_str"] == st.session_state.selected_date]["time_str"].unique())
...
for key, default in [("previous_focus_zone_id", None), ("use_3d_column", True), ("has_query", False), ("show_selected_detail", False), ("animate_zoom", False), ("last_llm_error", None)]:
...
if selected_date not in available_dates_all:
    selected_date = available_dates_all[0]
    st.session_state.selected_date = selected_date

available_times_selected = sorted(pred[pred["date_str"] == selected_date]["time_str"].unique())
if selected_time not in available_times_selected:
    selected_time = available_times_selected[0]
    st.session_state.selected_time = selected_time

pred_filtered = get_base_slice(pred, selected_date, selected_time, MAP_HORIZON)
if pred_filtered.empty:
    st.warning("선택한 날짜/시간에 해당하는 예측 데이터가 없습니다.")
    st.stop()

selected_dt = pred_filtered["base_datetime"].iloc[0]
target_dt = pred_filtered["target_datetime"].iloc[0]
...
metrics = calc_zone_metrics(pred, selected_date, selected_time, selected_zone_id)
selected_area = area_info[area_info["생활권역ID"] == selected_zone_id].copy()
if selected_area.empty:
    selected_label, selected_dongs = selected_zone_id, ""
else:
...
if st.session_state.has_query and st.session_state.show_selected_detail:
    selected_detail_html = build_selected_detail_html(selected_label, selected_zone_id, selected_dongs, metrics, selected_date, selected_time, model_label)
    ...
    with st.container(border=True, height=PANEL_HEIGHT):
        ...
        with model_col:
            model_choice = st.radio(
                "모델 선택",
                options=["일반 모델", "피크 모델"],
                index=0 if st.session_state.model_mode == "overall" else 1,
                horizontal=True,
        ...
        with chat_placeholder:
            render_chat_panel(st.session_state.messages, selected_detail_html, is_typing=False)
        ...
        if submitted and user_text.strip():
            ...
            parsed = parse_user_query(clean_user_text, pred, area_info, st.session_state.selected_date, st.session_state.selected_time, st.session_state.selected_zone_id)
            if not parsed["ok"]:
                answer = build_conversational_answer(clean_user_text, parsed["message"], parsed["reason"], parsed.get("llm_extract"), st.session_state.messages, pred, model_label)
                st.session_state.messages.append({"role": "assistant", "content": answer})
                st.session_state.show_selected_detail = False
                st.session_state.animate_zoom = False
                st.rerun()

            st.session_state.previous_focus_zone_id = st.session_state.selected_zone_id if st.session_state.has_query else None
            answer, answer_state = build_answer_from_parsed_query(clean_user_text, parsed, pred, area_info, st.session_state.messages, model_label)
            st.session_state.messages.append({"role": "assistant", "content": answer})
            ...
            st.session_state.show_selected_detail = bool(answer_state.get("show_selected_detail", True))
