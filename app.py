...
def draw_alerts_stack(top_df: pd.DataFrame, selected_time: str) -> None:
    ...
    <style>
        ...
        .ev-alert-card {{ height: 112px; display: grid; grid-template-columns: 62px minmax(0, 1fr) 106px; align-items: center; gap: 13px; background: #FFFFFF; border: 1.1px solid rgba(20,20,20,0.20); border-radius: 18px; padding: 14px 14px; box-sizing: border-box; margin-bottom: 20px; box-shadow: none; }}
...
with chat_col:
    with st.container(border=True, height=PANEL_HEIGHT):
        mark_panel()
        title_col, model_col = st.columns([0.40, 0.32], gap="small")
        with title_col:
